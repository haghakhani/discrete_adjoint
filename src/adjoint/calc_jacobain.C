			...
			while (currentPtr) {
				...
				if (Curr_El->get_adapted_flag() > 0) {
					...
					if (!boundary) {
						...
						Vec_Mat& jacobian = Curr_El->get_jacobian();
						// (side:x=0,y=1)(direction:neg=0,pos=1)
						FluxJac& flux_jac = (Curr_El->get_flx_jac_cont());

						Matrix<double>& h_slope_sens = Curr_El->get_hslope_sens();
						...
						for (int effelement = 0; effelement < EFF_ELL; effelement++) { //0 for the element itself, and the rest id for neighbour elements
							...
							if ((effelement == 0 && prev_state_vars[0] == 0.) || //this is a void element so the residual vector does not change by changing it's values
								...
								} else if ((effelement - 1) % 4 == xp || (effelement - 1) % 4 == xm) {
									dh_sens[1] = 0.;
								...
								} else if ((effelement - 1) % 4 == yp || (effelement - 1) % 4 == ym) {
									dh_sens[0] = 0.;
								...
								calc_jacobian_elem(jacobian(effelement), flux_jac(0, 0, index),
								    flux_jac(0, 1, index), flux_jac(1, 0, index), flux_jac(1, 1, index), state_vars,
								    d_state_vars, (d_state_vars + NUM_STATE_VARS), curvature, gravity, d_gravity,
								    dh_sens, matprops_ptr->intfrict, bedfrict, *(Curr_El->get_kactxy()), effelement,
								    dtdx, dtdy, dt, stop, orgSrcSgn);
