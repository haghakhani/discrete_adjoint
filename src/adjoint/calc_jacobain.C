...
int check_restore(double* prev_state_vars, double* d_state_vars,
		double* prev_state_vars_old, double* d_state_vars_old,
		double flux[][NUM_STATE_VARS], double fluxold[][NUM_STATE_VARS]);
			...
			while (currentPtr) {
				...
				if (Curr_El->get_adapted_flag() > 0) {
					...
					if (!boundary) {
						...
						get_flux(El_Table, NodeTable, Curr_El->pass_key(), matprops_ptr,
								myid, flux);
						...
						update_states(state_vars, prev_state_vars, flux[0], flux[1],
								flux[2], flux[3], dtdx, dtdy, dt, d_state_vars,
								(d_state_vars + NUM_STATE_VARS), curvature,
								matprops_ptr->intfrict, bedfrict, gravity, d_gravity,
								*(Curr_El->get_kactxy()), matprops_ptr->frict_tiny, stop,
								orgSrcSgn);
						...
						for (int effelement = 0; effelement < EFF_ELL; effelement++) { //0 for the element itself, and the rest id for neighbour elements
							...
							if (*(Curr_El->pass_key()) == KEY0
									...
									&& effelement == EFFELL)
								Curr_El->write_elem_info(NodeTable, filename,
							...
							if ((effelement == 0 && prev_state_vars[0] == 0.)
									...
									(effelement > 4
											&& *(Curr_El->get_neigh_proc() + (effelement - 1)) == -2)
									|| //one neighbor in this side
									(effelement != 0
											&& void_neigh_elem(El_Table, Curr_El, effelement)) || //this is a void neighbor element so the residual of the curr_el does not depend on this neighbor
									(effelement > 0
											&& *(Curr_El->get_neigh_proc() + (effelement - 1)) == INIT))
								...
								if (effelement == 0) {
									...
									std::cerr << "something is not correct here \n";
								}

								calc_jacobian_elem(jacobian(effelement), flux_jac(0, 0, index),
										flux_jac(0, 1, index), flux_jac(1, 0, index),
										flux_jac(1, 1, index), prev_state_vars, d_state_vars,
										(d_state_vars + NUM_STATE_VARS), curvature, gravity,
										d_gravity, dh_sens, matprops_ptr->intfrict, bedfrict,
										*(Curr_El->get_kactxy()), effelement, dtdx, dtdy, dt, stop,
										orgSrcSgn);
...
void restore(HashTable* El_Table, HashTable* NodeTable, Element* Curr_El,
	...
	if (effelement == 0) { //this part of code add an increment to the state variables to find the Jacobian, but the problem is since it is called after correct, the increment shoud be added to the prev_state_vars
		...
		Curr_El->calc_edge_states(El_Table, NodeTable, matprops_ptr, myid, dummydt,
				&order_flag, &outflow); //chenge of the state_vars causes the all around fluxes change, this update xp ,yp
		...
		if ((*(Curr_El->get_neigh_proc() + xm)) != INIT) { //we have to make sure that there exit an element in xm side
			...
			elem_xm->calc_edge_states(El_Table, NodeTable, matprops_ptr, myid,
					dummydt, &order_flag, &outflow); //this update the flux on share edge with xm
		}
		if ((*(Curr_El->get_neigh_proc() + ym)) != INIT) { //we have to make sure that there exit an element in ym side
			...
			elem_ym->calc_edge_states(El_Table, NodeTable, matprops_ptr, myid,
					dummydt, &order_flag, &outflow); //this update the flux on share edge with ym
		}
		...
		*(neigh_elem->get_prev_state_vars() + j) -= increment;
		...
		if ((effelement - 1) == xp || (effelement - 1) == yp)
			Curr_El->calc_edge_states(El_Table, NodeTable, matprops_ptr, myid,
					dummydt, &order_flag, &outflow); //if we change the state variables in xp or yp, just the flux at this element has to be updated
		else
			neigh_elem->calc_edge_states(El_Table, NodeTable, matprops_ptr, myid,
					dummydt, &order_flag, &outflow); //otherwise the flux at the corresponding element has to be updated
...
void restore(HashTable* El_Table, HashTable* NodeTable, Element* Curr_El,
		int effelement, int j, double increment, double fluxold[4][NUM_STATE_VARS],
...
void increment_state(HashTable* El_Table, Element* Curr_El, double increment,
		int effelement, int j, int* updateflux, int* srcflag,
		ResFlag resflag[EFF_ELL]) {
...
//										}
	...
	if (effelement == 0 && updateflux) { //this part of code add an increment to the state variables to find the Jacobian, but the problem is since it is called after correct, the increment shoud be added to the prev_state_vars
		...
		Curr_El->calc_edge_states(El_Table, NodeTable, matprops_ptr, myid, dummydt,
				&order_flag, &outflow); //change of the state_vars causes the all around fluxes change, this update xp ,yp
		// earlier in this file, we made sure that this element is not a boundary element
		// so here we do not require to check the neighbor elements to make sure they are not located on the boundary
		...
		elem_xm->calc_edge_states(El_Table, NodeTable, matprops_ptr, myid, dummydt,
				&order_flag, &outflow);		//this update the flux on share edge with xm

		Element* elem_ym = (Element*) (El_Table->lookup(
				Curr_El->get_neighbors() + ym * KEYLENGTH));
		assert(elem_ym);
		elem_ym->calc_edge_states(El_Table, NodeTable, matprops_ptr, myid, dummydt,
				&order_flag, &outflow);		//this update the flux on share edge with ym
