...
void flux_debug(Element* Curr_El, double fluxxpold[4], double fluxxmold[4],
	...
	double fluxxold[state_num], fluxxnew[state_num], fluxyold[state_num],
			fluxynew[state_num];
	...
	for (int ivar = 0; ivar < state_num; ivar++) {
		...
		if (abs_fluxx_diff[ivar] > 0. && ivar != 1)
			cout << setw(10) << setprecision(8)
					<< "change effects the flux_x for var  " << ivar << " eff_elem   "
					<< effelement << "  j  " << j << "  value  " << abs_fluxx_diff[ivar]
					<< endl;

		abs_fluxy_diff[ivar] = dabs(fluxxold[ivar] - fluxxnew[ivar]);
		if (abs_fluxy_diff[ivar] > 0. && ivar != 1)
			cout << "change effects the flux_y for var  " << ivar << " eff_elem   "
					<< effelement << "  j  " << j << "  value  " << abs_fluxy_diff[ivar]
					<< endl;
	}
