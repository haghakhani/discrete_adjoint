...
c         tttest = sgn(vel, tiny)
         vel=(dUdx(3)/uvec(1) - uvec(3)*dudx(1)/uvec(1)**2.d0+
     1        dUdy(4)/uvec(1) - uvec(4)*dudy(1)/uvec(1)**2.d0)
...
c     if there is no yielding...
         if(dabs(uvec(3)/uvec(1)) .lt. tiny .and.
     1        dabs(uvec(4)/uvec(1)) .lt. tiny) then
            kactx = 1.d0
            kacty = 1.d0
         endif
      else 
