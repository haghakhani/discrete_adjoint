...
void move_err_data(MeshCTX* meshctx, PropCTX* propctx) {
	...
	int send_tag = 22674; //original value Keith didn't change
	ErrElemPack** send_array = new ErrElemPack*[numprocs];
	ErrElemPack** recv_array = new ErrElemPack*[numprocs];

	for (iproc = 0; iproc < numprocs; iproc++) {
		IfRecvDone[iproc] = !num_send_recv[iproc];
		if (num_send_recv[iproc] > 0) {
			send_array[iproc] = new ErrElemPack[num_send_recv[iproc]];
			recv_array[iproc] = new ErrElemPack[num_send_recv[iproc]];

			ierr = MPI_Irecv((void*) recv_array[iproc], num_send_recv[iproc], ERRELEMTYPE, iproc,
			    send_tag + iproc, MPI_COMM_WORLD, &(RequestRecv[iproc]));
		} //if(num_send_recv[iproc] != 0)
	} //for(iproc=0;iproc<numprocs;iproc++)
	...
	for (iproc = 0; iproc < numprocs; iproc++) {
		...
		if (num_send_recv[iproc] > 0)
			//send ghost elements to processor iproc
			ierr = MPI_Isend((void*) send_array[iproc], num_send_recv[iproc], ERRELEMTYPE, iproc,
			    send_tag + myid, MPI_COMM_WORLD, &(RequestSend[iproc]));
	}

	int NumNotRecvd, IfSentRecvd;
	MPI_Status status;
	ErrorElem *elm, *new_elm;
	double not_used, *dPtr, *d2Ptr;
	int add_counter = 0, update_counter = 0;
