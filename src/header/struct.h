...
struct ElemPack {
	//see ../repartition/new_datatype.C blockcounts[3]={36,25*KEYLENGTH,59}
	int myprocess;                                      //  1
	int generation;                                     //  2
	int material;                                       //  3
	int neigh_proc[8];                                  // 11
	...
	unsigned key[KEYLENGTH];/*9th node key*/            //  1
	...
	double dx[DIMENSION];                               // 35
	double kactxy[DIMENSION];                           // 37
	double zeta[DIMENSION];                             // 39
	double curvature[DIMENSION];                        // 41
	double gravity[3];                                  // 44
	double d_gravity[DIMENSION];                        // 46
	double lb_weight;                                   // 47
	double node_elevation[9];                           // 56
	double Influx[NUM_STATE_VARS];                      // 59
};

// this structures is exactly same as ElemPack but has some more data
struct DualElemPack {
	//see ../repartition/new_datatype.C blockcounts[3]={36,25*KEYLENGTH,65}
	int myprocess;                                      //  1
	int generation;                                     //  2
	int material;                                       //  3
	int neigh_proc[8];                                  // 11
...
// this structures is exactly same as ElemPack but has some more data
struct ErrElemPack {
	//see ../repartition/new_datatype.C blockcounts[3]={36,25*KEYLENGTH,71}
	int myprocess;                                     //  1
	int generation;                                    //  2
	int material;                                      //  3
	int neigh_proc[8];                                 // 11
	...
	double dx[DIMENSION];                              // 38
	double kactxy[DIMENSION];                          // 40
	double zeta[DIMENSION];                            // 42
	double curvature[DIMENSION];                       // 44
	double gravity[NUM_STATE_VARS];                    // 47
	double d_gravity[DIMENSION];                       // 49
	double lb_weight;                                  // 50
	double node_elevation[9];                          // 59
	double Influx[NUM_STATE_VARS];                     // 62
	double bilin_adj[NUM_STATE_VARS];                  // 65
	double bilin_state[NUM_STATE_VARS];                // 68
	double bilin_prev_state[NUM_STATE_VARS];           // 71
};
