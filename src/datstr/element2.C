...
//l="left" (the minus side), r="right" (the plus side)
	...
	if ((hfvl(0, 0) == 0.) && (hfvr(0, 0) == 0.)) {
		...
		if (sl >= 0.) {
			...
			//---------------------------------------
//			double d_flux_d_Fr = sr * inv_speed_diff;
//			double d_flux_d_Fl = -sl * inv_speed_diff;
			...
			for (int i = 0; i < NUM_STATE_VARS; ++i)
				for (int j = 0; j < NUM_STATE_VARS; ++j) {
					...
					jac_r(i, j) = d_flux_d_sr[i] * d_sr_r[j] + d_flux_d_sl[i] * d_sl_r[j]
							+ flux_jac_r(i, j) + jac_v_diff_r(i, j);

					jac_l(i, j) = d_flux_d_sr[i] * d_sr_l[j] + d_flux_d_sl[i] * d_sl_l[j]
							+ flux_jac_l(i, j) + jac_v_diff_l(i, j);
				}
...
void Element::calc_flux(HashTable* El_Table, HashTable* NodeTable, int myid,
		int side) {
